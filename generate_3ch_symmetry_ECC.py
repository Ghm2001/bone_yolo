...
#   图像配准函数（ECC）
# ======================
def register_images(fixed, moving, warp_mode=cv2.MOTION_AFFINE, number_of_iterations=500, termination_eps=1e-6):
    """使用 OpenCV ECC 算法进行图像配准。 fixed: 原图（基准） moving: 镜像图（待对齐） 返回: 对齐后的 moving 图像.
    """
    ...
    try:
        _cc, warp_matrix = cv2.findTransformECC(fixed_f, moving_f, warp_matrix, warp_mode, criteria)
        sz = fixed.shape
        if warp_mode == cv2.MOTION_HOMOGRAPHY:
            aligned = cv2.warpPerspective(
                moving, warp_matrix, (sz[1], sz[0]), flags=cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP
            )
        else:
            aligned = cv2.warpAffine(moving, warp_matrix, (sz[1], sz[0]), flags=cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP)
        return aligned
        ...
        for image_path in source_path.iterdir():
            if image_path.suffix.lower() in [".png", ".jpg", ".jpeg", ".tiff"]:
                ...
                if img_3ch is not None:
                    ...
                    try:
                        os.link(labels_path / label_filename, output_labels_path / label_filename)
